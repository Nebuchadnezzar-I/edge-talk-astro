import { A as AstroError, T as ActionNotFoundError } from './astro/assets-service_CfhXJX0c.mjs';

async function getAction(path) {
  const pathKeys = path.replace("/_actions/", "").split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_BVmV0wZl.mjs');
